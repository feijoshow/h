...
const PestIdentifier: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    setResult(null);
    try {
      const analysisResult = await identifyPest(imageFile);
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-brand-dark-green mb-2">Pest & Insect Identifier</h2>
      <p className="text-lg text-stone-600 mb-6">Upload an image of a pest or insect to get identification and control methods.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        <div className="flex flex-col items-center">
          ...
          >
            {isLoading ? <Loader /> : <BugIcon className="w-5 h-5" />}
            {isLoading ? "Identifying..." : "Identify Pest"}
          </button>
        ...
        <div>
          {isLoading && !result && (
             <div className="flex flex-col items-center justify-center text-center p-8 bg-brand-light rounded-lg h-full">
              <Loader />
              <p className="mt-4 text-lg text-brand-dark-green font-semibold">Identifying subject...</p>
          ...
          {result && (
            <div className="bg-brand-light p-6 rounded-lg shadow-inner animate-fade-in">
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-semibold text-stone-500 uppercase tracking-wider">Identification</h4>
                  <p className="text-xl text-brand-brown font-bold">{result.pestName}</p>
                </div>
                 <div>
                    <h4 className="text-sm font-semibold text-stone-500 uppercase tracking-wider">Harmful to Crops?</h4>
                    <p className={`text-xl font-bold ${result.isHarmful ? 'text-red-600' : 'text-green-600'}`}>
                        {result.isHarmful ? "Yes" : "No"}
                    </p>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-stone-500 uppercase tracking-wider">Description & Potential Damage</h4>
                  <p className="text-stone-700">{result.damageDescription}</p>
                </div>

                {result.controlMethods && result.controlMethods.length > 0 && (
                  <div>
                    <h4 className="text-lg font-bold text-brand-dark-green mt-6 mb-2">Suggested Control Methods</h4>
                    <ul className="space-y-3">
                      {result.controlMethods.map((method, index) => (
                        <li key={index} className="bg-white p-4 rounded-md shadow">
                          <p className="font-bold text-brand-green">{method.method}</p>
